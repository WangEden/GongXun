...
在暂存区垛码放置物料
"""
def fineTuneRing3(threshold:list, threshold2:list, ringRate:float):
    ...
    while flag:
        ...
        maskGreenItem = cv2.inRange(img_hsv, threshold2[1][0], threshold2[1][1])
        maskGreenItem = cv2.medianBlur(maskGreenItem, 3)

        maskGreen = maskGreenRing + maskGreenItem
        ...
        #     cv2.imwrite(f"./data/t42ringwt/查找的绿色色环mask{k}.jpg", maskGreenRing)
        #     cv2.imwrite(f"./data/t42ringwt/查找的绿色色环{k}.jpg", img_note)
        # elif loop == 2:
        cv2.imwrite("./data/t72ringwt/最后一帧.jpg", img)
        cv2.imwrite(f"./data/742ringwt/查找的绿色色环mask{k}.jpg", maskGreenRing)
